...
(function(){
    try {
        ...
        if (!isLocal) {
            ...
            // console.error 는 진짜 에러 추적 위해 남겨둠
        }
    } catch(e) {}
})();
...
function cleanOldCache() {
    try {
        var now = new Date();
        var curYM = now.getFullYear() * 12 + now.getMonth(); // 0-base 월
        var KEEP_MONTHS = 12;
        var removed = 0;
        // localStorage 키를 배열로 복사 후 순회 (삭제 중 인덱스 꼬임 방지)
        var keys = [];
        for (var i = 0; i < localStorage.length; i++) keys.push(localStorage.key(i));
        keys.forEach(function(k){
            if (!k) return;
            // cal_cache_out_2026_5 / yearly_stats_cache_in_2025 형태 매칭
