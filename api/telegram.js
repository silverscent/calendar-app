...
module.exports = async function handler(req, res) {
    try {
        ...
        if (cmdList.includes(text.split(' ')[0])) {
            ...
            let fetchStart = new Date(startDate); fetchStart.setDate(fetchStart.getDate() - 10);
        ...
        if (text === '/출고') {
            ...
            let futureGroups = {}; 
            let mainRackPallets = 0; let otherLocationPallets = 0; const RACK_CAPA = 48; 
            
            rows.forEach(r => {
                let company = String(r.company).trim(); let etc = String(r.etc).trim();
                let pCount = parseInt(r.pal) || 0; let bCount = parseInt(r.box) || 0;
                let rawDateStr = "미정";
                ...
                if (!isTaskMode) {
                    if (isOtherLocation) otherLocationPallets += pCount; else mainRackPallets += pCount;
                }
                let cleanCompany = company.replace(/\[TASK\]/gi, '').trim();
                let etcText = etc ? ` 📍${etc}` : ''; let qtyText = '';
                if (!isTaskMode) {
                    if (pCount > 0 && bCount > 0) qtyText = ` | ${pCount}P (${bCount}B)`;
                    else if (pCount > 0) qtyText = ` | ${pCount}P`;
                    else if (bCount > 0) qtyText = ` | ${bCount}B`;
                }
                let itemStr = isTaskMode ? ` 📌 [작업] ${cleanCompany}${qtyText}${etcText}` : ` ${rawDateStr === "미정" ? "📦" : "🚛"} ${cleanCompany}${qtyText}${etcText}`;
            ...
            msg += `━━━━━━━━━━━━━━━━━━━━━\n📅 [주간 출고 & 작업 타임라인]\n\n`;
            let sortedDates = Object.keys(futureGroups).sort(); let hasFuture = false; const dayNames = ["일","월","화","수","목","금","토"];
            sortedDates.forEach(d => {
                hasFuture = true; let dateObj = new Date(d); let weekStr = dayNames[dateObj.getDay()];
            ...
            if (pendingCargoList.length > 0 || pendingTaskList.length > 0) {
                hasFuture = true; msg += `⏳ [일정 미정 (대기 및 보류)]\n`;
        ...
        if (text.startsWith('/달력') || text.startsWith('/calendar') || text.startsWith('/출고달력') || text.startsWith('/용차달력')) {
            ...
            const year = targetDate.getFullYear(); const month = targetDate.getMonth() + 1; const dayNames = ["일","월","화","수","목","금","토"]; 
            ...
            if (isOutbound) {
                ...
                rows.forEach(r => {
                    let dDate = new Date(r.outbound_date); let dStr = `${month}/${String(dDate.getDate()).padStart(2, '0')}(${dayNames[dDate.getDay()]})`;
                    if (currentD !== dStr) {
                ...
                let currentD = ''; let dailyPal = 0; let dailyStr = '';
                rows.forEach(r => {
                    let dDate = new Date(r.receive_date); let dStr = `${month}/${String(dDate.getDate()).padStart(2, '0')}(${dayNames[dDate.getDay()]})`;
                    if (currentD !== dStr) {
                        if (currentD !== '') calStr += `📦 ${currentD} - 총 ${dailyPal}p\n${dailyStr}\n`;
                        currentD = dStr; dailyPal = 0; dailyStr = '';
                    }
                    ...
                    let sTypeRaw = String(r.s_type || '').toUpperCase(); let sType = sTypeRaw === "AIR" ? "✈️ 탑차량" : (sTypeRaw === "SEA" ? "🚢 컨테이너" : sTypeRaw);
                ...
                pendingCount = pendings.length;
                pendings.forEach(p => { 
                    let sTypeRaw = String(p.s_type || '').toUpperCase(); let sType = sTypeRaw === "AIR" ? "✈️ 탑차량" : (sTypeRaw === "SEA" ? "🚢 컨테이너" : sTypeRaw);
        ...
        if (isAdminCmd || isImageUpload) {
            ...
            if (text.startsWith('/dup')) {
                if (text.includes('on')) {
                    ...
                    const val = '"OFF"'; await pool.query(`INSERT INTO system_settings (setting_key, setting_value) VALUES ('DUP_OPTION', ?) ON DUPLICATE KEY UPDATE setting_value=?`, [val, val]);
                    await sendTgMsg(chatId, '🔓 중복 이미지 허용 모드가 활성화되었습니다.');
                } else if (text.includes('reset')) {
                    await pool.query(`DELETE FROM processed_images`); await sendTgMsg(chatId, '♻ 중복 이미지 기록을 모두 초기화했습니다.');
                }
                return res.status(200).send('OK');
            }
            ...
            if (text.startsWith('/완료') || text.startsWith('/처리')) {
                const parts = text.trim().split(/\s+/);
                if (parts.length < 2) { await sendTgMsg(chatId, "⚠️ 사용법: /완료 [B/L 또는 인보이스] (옵션: 어제/0302)"); return res.status(200).send('OK'); }
                ...
                let exist = [];
                if (targetStr !== '발행전') { [exist] = await pool.query(`SELECT id, remarks, bl_number, invoice FROM inbound WHERE TRIM(bl_number) = ? OR TRIM(invoice) = ? ORDER BY id DESC LIMIT 1`, [targetStr, targetStr]); } 
                else { await sendTgMsg(chatId, "⚠️ '발행전'은 인보이스 번호로 검색해주세요!"); return res.status(200).send('OK'); }

                if (exist.length > 0) {
                    let etc = String(exist[0].remarks || '').trim(); if (!etc.includes('[수동완료]')) etc = etc ? etc + ' [수동완료]' : '[수동완료]';
            ...
            if (text.startsWith('/일괄완료')) {
                const parts = text.trim().split(/\s+/); const now = new Date(new Date().toLocaleString("en-US", {timeZone: "Asia/Seoul"}));
                let dateToSet = new Date(now.getFullYear(), now.getMonth(), now.getDate());
                ...
                if (parts[1]) {
                    if (parts[1] === '어제') dateToSet.setDate(dateToSet.getDate() - 1);
                        ...
                        if (match) dateToSet = new Date(now.getFullYear(), parseInt(match[1]) - 1, parseInt(match[2]));
                        else { await sendTgMsg(chatId, "⚠️ 날짜 형식이 잘못되었습니다."); return res.status(200).send('OK'); }
                    }
                }
                const dateStr = getKstDateStr(dateToSet);
                ...
                if (pendings.length === 0) { await sendTgMsg(chatId, "⚠️ 누락 화물(미정)이 DB에 없습니다."); return res.status(200).send('OK'); }

                for (const p of pendings) {
                    let etc = String(p.remarks || '').trim(); if (!etc.includes('[일괄완료]')) etc = etc ? etc + ' [일괄완료]' : '[일괄완료]';
            ...
            if (text.startsWith('/용차')) {
                ...
                const company = parts[0]; let dateStr = "미정"; let numbers = []; let etcParts = []; let explicitPal = null; let explicitBox = null;
                for (let i = 1; i < parts.length; i++) {
                    const p = parts[i]; let isDate = false; let mStr, dStr;
                    if (/^\d{4}$/.test(p)) {
                        ...
                        const match = p.match(/^(0?[1-9]|1[0-2])[-/]([0-2][0-9]|3[01])$/); mStr = parseInt(match[1], 10); dStr = parseInt(match[2], 10); isDate = true;
                    }
                    const now = new Date(new Date().toLocaleString("en-US", {timeZone: "Asia/Seoul"}));
                    ...
                    if (numMatch) {
                        ...
                        else numbers.push(val); continue;
                    }
                    etcParts.push(p);
                }
                
                let box = explicitBox !== null ? explicitBox : ""; let pal = explicitPal !== null ? explicitPal : "";
                if (numbers.length === 2) {
                    if (pal === "") pal = Math.min(numbers[0], numbers[1]); if (box === "") box = Math.max(numbers[0], numbers[1]);
                } else if (numbers.length === 1) {
                    if (pal === "" && box !== "") pal = numbers[0]; else if (box === "" && pal !== "") box = numbers[0]; else { box = numbers[0]; pal = ""; } 
                }
                ...
                if (exist.length > 0) {
                    ...
                    if (numbers.length === 1 && explicitPal === null && explicitBox === null) {
                        ...
                        else if (existingBox !== "" && existingPal === "") { pal = numbers[0]; box = existingBox; } else { box = numbers[0]; pal = existingPal; }
                    } else if (pal === "" && box === "") { pal = existingPal; box = existingBox; } else { if(pal === "") pal = existingPal; if(box === "") box = existingBox; }
                    const finalDate = dateStr !== "미정" ? dateStr : exist[0].outbound_date; const finalEtc = etcStr !== "" ? etcStr : exist[0].etc;
            ...
            if (text.startsWith('/이동') || text.startsWith('/위치')) {
                const parts = text.trim().split(/\s+/); if (parts.length < 3) { await sendTgMsg(chatId, "⚠️ 사용법: /이동 [업체명] [장소]"); return res.status(200).send('OK'); }
            ...
            if (text.startsWith('/출고완료') || text.startsWith('/용차완료')) {
                const parts = text.trim().split(/\s+/); if (parts.length < 2) { await sendTgMsg(chatId, "⚠️ 사용법: /출고완료 [업체명]"); return res.status(200).send('OK'); }
                const company = parts[1];
                const [exist] = await pool.query(`SELECT * FROM outbound WHERE company = ? AND isDone = 0 ORDER BY id DESC LIMIT 1`, [company]);
                if (exist.length > 0) {
                    let etc = String(exist[0].etc || '').trim(); let newEtc = etc ? etc + ' [출고완료]' : '[출고완료]';
            ...
            if (text.startsWith('/출고삭제') || text.startsWith('/용차삭제')) {
                const parts = text.trim().split(/\s+/); if (parts.length < 2) { await sendTgMsg(chatId, "⚠️ 사용법: /출고삭제 [업체명]"); return res.status(200).send('OK'); }
                const company = parts[1];
