...
function searchRawDatabaseRows() {
          ...
          apiCall({
              ...
              const th = (col, name) => `<th onclick="setDbSort('${col}')" style="cursor:pointer; user-select:none; white-space:nowrap;" title="클릭하여 정렬">${name} <span style="font-size:0.8em">${getSortIcon(col)}</span></th>`;
...
function refreshAdminList() {
          ...
          if (!container) return;
          container.innerHTML = "<div style='color:var(--text-sub); text-align:center; padding:15px; font-weight:800;'>보안 세션 확인 중...</div>";

          apiCall({ source: 'vercel', action: 'GET_ADMIN_LIST' }).then(function(res) {
              if (res === null || !res.success || !res.list) {
                  container.innerHTML = "<div style='color:var(--text-sub); padding:10px;'>목록 수집 실패</div>"; return;
              }
              container.innerHTML = "";
              ...
              const buildCard = (item, isLocked) => {
                  ...
                  const statusBadge = isLocked ? `<span style="font-size:0.7em; color:#ff3b30; border:1px solid #ff3b30; padding:2px 4px; border-radius:4px; font-weight:bold;">비활성</span>` : "";
              ...
              if (lockedUsers.length > 0) {
                  ...
                  divider.style = "text-align:center; margin:25px 0 15px 0; font-size:0.8em; color:#ff3b30; font-weight:800; display:flex; align-items:center; gap:10px; user-select:none;";
...
function bindBioAuthEvents() {
    ...
    if (bioCancelBtn) {
        bioCancelBtn.onclick = function() { closeBioPwModal(); };
    }
    if (bioPwInput) {
            ...
            try {
                ...
                .then(function(credential) {
                    ...
                    (async function() {
                        ...
                        if (res === null) { showToast("⚠️ 서버 연결에 실패했습니다.", 2500); return; }
                        if (res.success) {
                            ...
                            alert("비밀번호가 일치하지 않습니다. 등록이 취소됩니다.\n(※ 기기에 방금 저장된 패스키는 무효 처리되므로 삭제 권장)");
                ...
                .catch(function(err) {
                    ...
                    alert("⚠️ 생체 인증 거부 및 실패: " + err.message + "\n(도메인 보안 정책 또는 IP 주소 접속 여부를 확인하세요)");
                    const toggle = document.getElementById('toggleBioAuth');
                    if (toggle) toggle.checked = false;
                });
...
function initNativeBottomSheet() {
          document.querySelectorAll('.overlay-modal').forEach(modal => {
              ...
              attach(handle);
              attach(header);
              box.addEventListener('touchstart', (e) => { if(e.target === box) startDrag(e); }, { passive: true });
              box.addEventListener('mousedown', (e) => { if(e.target === box) startDrag(e); });
          });
...
function refreshAuditLogs() {
          ...
          timeline.innerHTML = "<div style='color:var(--text-sub); text-align:center; padding:15px; font-weight:800;'>보안 로그 동기화 및 검색 중...</div>";
          if (pager) pager.innerHTML = "";
          ...
          apiCall({
              ...
              if (res === null || !res.success || !res.logs || res.logs.length === 0) {
                  timeline.innerHTML = "<div style='color:var(--text-sub); text-align:center; padding:20px;'>검색 조건에 맞는 감사 로그가 없습니다.</div>";
                  return;
              }
              ...
              res.logs.forEach(log => {
                  ...
                  item.style = "padding-bottom:12px; border-bottom:1px solid var(--border-color); font-size:0.9em; box-sizing:border-box; margin-bottom:10px;";
...
function refreshConnLogs() {
          ...
          const keyword = document.getElementById('logConnKeyword') ? document.getElementById('logConnKeyword').value.trim() : '';
          const limit = document.getElementById('logConnLimit') ? document.getElementById('logConnLimit').value : 100;

          timeline.innerHTML = "<div style='color:var(--text-sub); text-align:center; padding:15px; font-weight:800;'>접속망 트래킹 중... ⏳</div>";
          if (pager) pager.innerHTML = "";
          ...
          apiCall({
              ...
              if (res === null || !res.success || !res.logs || res.logs.length === 0) {
                  timeline.innerHTML = "<div style='color:var(--text-sub); text-align:center; padding:20px;'>검색 조건에 맞는 접속 로그가 없습니다.</div>";
                  return;
              }
...
function toggleAdmin() {
          if(isAdmin) {
              ...
              const isMasterUser = savedId.toLowerCase() === 'admin' || 
                                   savedId.toLowerCase() === 'silverscent' ||
                                   (savedRole && (savedRole.toUpperCase().includes('SUPER') || 
              ...
              let isBioRegistered = localStorage.getItem('bio_registered') === 'true' && localStorage.getItem('bio_id') === savedId;
...
function saveFullRowDB(id) {
          ...
          apiCall({ source: 'vercel', action: 'UPDATE_RAW_ROW_FULL', updateData: updateData, type: currentType, admin_id: localStorage.getItem('admin_id') })
          .then(function(res) {
              if (res === null) return;
              if (res.success) {
...
function addNewRowDirectDB(type) {
          ...
          if (type === 'out') {
              ...
              if (!insertData.company) { showToast("⚠️ 업체명은 필수입니다.", 2000); return; }
          } else {
              insertData = {
              ...
              if (!insertData.bl_number) { showToast("⚠️ B/L 번호는 필수입니다.", 2000); return; }
          }

          apiCall({ source: 'vercel', action: 'ADD_RAW_ROW_DIRECT', type: currentType, insertData: insertData, admin_id: localStorage.getItem('admin_id') })
          .then(function(res) {
              if (res === null) return;
              if (res.success) {
...
function handleLogin() {
    ...
    apiCall({ source: 'vercel', action: 'LOGIN', data: { id, pw } }).then(function(res) {
        if (loginBtn) { loginBtn.innerHTML = originalBtnText; loginBtn.style.pointerEvents = 'auto'; loginBtn.style.opacity = '1'; }
        if (res === null) { showToast(`❌ 통신 오류가 발생했습니다.`, 2500); return; }
...
function executeMyPasswordChange() {
          ...
          if (!curPw || !newPw) { showToast("⚠️ 현재 비밀번호와 새 비밀번호를 모두 입력하세요.", 2000); return; }
          if (curPw === newPw) { showToast("⚠️ 새 비밀번호는 현재 비밀번호와 다르게 입력해 주세요.", 2000); return; }
          if (!confirm("정말 비밀번호를 변경하시겠습니까?")) return;

          showToast("⏳ 보안 검증 및 변경 중...", 0);
          
          
          apiCall({ source: 'vercel', action: 'CHANGE_MY_PASSWORD', data: { currentPw: curPw, newPw: newPw }, admin_id: localStorage.getItem('admin_id') })
          .then(function(res) {
              if (res === null) return;
              if (res.success) {
...
function createNewAdminAccount() {
          ...
          apiCall({ source: 'vercel', action: 'CREATE_ADMIN', data: { id, name, pw: pw }, admin_id: localStorage.getItem('admin_id') })
          .then(function(res) {
              if (res === null) return;
              if (res.success) {
...
function saveAuthData(id, role, isSave, session_token) {
          ...
          if (isSave) {
              otherStorage.removeItem('isAdmin'); otherStorage.removeItem('admin_id'); otherStorage.removeItem('admin_role'); otherStorage.removeItem('session_token');
              ...
              localStorage.removeItem('isAdmin'); localStorage.removeItem('admin_id'); localStorage.removeItem('admin_role'); localStorage.removeItem('session_token');
              sessionStorage.removeItem('isAdmin'); sessionStorage.removeItem('admin_id'); sessionStorage.removeItem('admin_role'); sessionStorage.removeItem('session_token');
...
function renderOcrFilterBadges() {
    ...
    if (globalOcrFilters.length === 0) {
        container.innerHTML = "<div style='color:var(--text-sub); font-size:0.85em; font-weight:800;'>등록된 단어가 없습니다.</div>";
        return;
    }
...
function editRowDirectFromDB(rowId, oldName) {
          ...
          apiCall({ source: 'vercel', action: 'UPDATE_RAW_ROW_DIRECT', rowId: rowId, type: currentType, newName: newName.trim(), admin_id: localStorage.getItem('admin_id') })
          .then(function(res) {
              if (res === null) return;
              if (res.success) {
...
function switchMasterTab(tabId) {
          ...
          const activeBtn = Array.from(document.querySelectorAll('.master-tab-btn')).find(b => b.getAttribute('onclick').includes(tabId));
...
function deleteRowDirectFromDB(rowId) {
          if (!confirm(`⚠️ [초긴급 경고]\n데이터베이스 일련번호 [ID: ${rowId}] 로우를 영구 파기합니까?\n달력 동기화 데이터 꼬임 현상을 정밀 강제 수정할 때만 실행하세요.`)) return;
          
          showToast("💥 DB 로우 레벨 파괴 중...", 0);
          apiCall({ source: 'vercel', action: 'DELETE_RAW_ROW_DIRECT', rowId: rowId, type: currentType, admin_id: localStorage.getItem('admin_id') })
          .then(function(res) {
              if (res === null) return;
              if (res.success) {
...
function toggleNewAdminForm() {
          ...
          if (form.style.display === 'none' || !form.style.display) {
              ...
              btn.style.background = 'rgba(255,59,48,0.1)'; btn.style.color = '#ff3b30'; btn.style.borderColor = '#ff3b30';
          } else {
              form.style.display = 'none';
              btn.innerHTML = '➕ 신규 어드민 계정 발급실 열기';
              btn.style.background = 'rgba(10, 132, 255, 0.1)'; btn.style.color = '#0a84ff'; btn.style.borderColor = '#0a84ff';
          }
      }
...
function resetAdminPassword(id, name) {
          ...
          apiCall({ source: 'vercel', action: 'RESET_ADMIN_PW', data: { targetId: id, newPw: newPw }, admin_id: localStorage.getItem('admin_id') })
          .then(function(res) {
              if (res === null) return;
...
function hardDeleteAdminAccount(id, name) {
          if (!confirm(`🚨 최후 경고: [${name}(${id})] 계정을 데이터베이스에서 완전히 삭제(Hard Delete)합니까?\n이 작업은 되돌릴 수 없으며 접속 기록 외 모든 정보가 파기됩니다!`)) return;
          showToast("💥 계정 영구 파기 중...", 0);
          apiCall({ source: 'vercel', action: 'HARD_DELETE_ADMIN', data: { id: id }, admin_id: localStorage.getItem('admin_id') })
          .then(function(res) {
              if (res === null) return;
              if (res.success) { showToast("🗑️ 계정이 DB에서 완전히 파기되었습니다.", 2000); refreshAdminList(); }
              else { showToast(`❌ 삭제 실패: ${res.msg}`, 3000); }
          });
      }
...
function openLoginModal() {
          ...
          setTimeout(() => { if(idInput) idInput.focus(); else document.getElementById('adminPwInput').focus(); }, 150);
      }

function deleteAdminAccount(id, name) {
          if (!confirm(`[확인] [${name}(${id})] 관리자 권한을 회수하고 계정을 비활성화 하시겠습니까?`)) return;
          showToast("⏳ 권한 회수 중...", 0);
          apiCall({ source: 'vercel', action: 'DELETE_ADMIN', data: { id: id }, admin_id: localStorage.getItem('admin_id') })
          .then(function(res) {
              if (res === null) return;
              if (res.success) { showToast("✅ 계정이 비활성화 상태로 전환되었습니다.", 2000); refreshAdminList(); }
              else { showToast(`❌ 처리 실패: ${res.msg}`, 3000); }
          });
      }
...
function reactivateAdminAccount(id, name) {
          ...
          showToast("⏳ 계정 복구 중...", 0);
          apiCall({ source: 'vercel', action: 'REACTIVATE_ADMIN', data: { id: id }, admin_id: localStorage.getItem('admin_id') })
          .then(function(res) {
              if (res === null) return;
              if (res.success) { showToast("✅ 관리자 계정이 활성화되었습니다.", 2000); refreshAdminList(); }
              else { showToast(`❌ 복구 실패: ${res.msg}`, 3000); }
          });
      }
...
function addOcrFilterWord() {
    ...
    if (!word) return;
    if (globalOcrFilters.includes(word)) { showToast("이미 등록된 단어입니다.", 1500); return; }
...
function saveOcrFilterWordsToServer() {
    showToast("저장 중...", 0);
    apiCall({ source: 'vercel', action: 'SAVE_OCR_FILTERS', data: globalOcrFilters })
    .then(function(res) {
...
function loadOcrFilterWords() {
    apiCall({ source: 'vercel', action: 'GET_OCR_FILTERS' })
    .then(function(res) {
        if (res === null) return;
